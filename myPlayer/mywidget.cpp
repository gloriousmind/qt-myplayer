...
void MyWidget::initPlayer()
{
    ...
    skipBackwardAction->setIcon(QIcon(":/image/images/skipBackward.png"));
    ...
    skipForwardAction->setIcon(QIcon(":/image/images/skipForward.png"));
    ...
    openAction->setIcon(QIcon(":/image/images/open.png"));
    openAction->setShortcut(QKeySequence("Ctrl+O"));
    connect(openAction, SIGNAL(triggered()), this, SLOT(openFile()));
    //控制音量部件
    QSlider * volumeSlider = new QSlider(Qt::Horizontal, this);
    mediaObject_ctrl_playback->setVolume(50);
    ...
    connect(mediaObject_ctrl_playback, SIGNAL(stateChanged(QMediaPlayer::State)), this, SLOT(stateChanged(QMediaPlayer::State)));
    connect(mediaObject_ctrl_playback, SIGNAL(mediaStatusChanged(QMediaPlayer::MediaStatus)), this, SLOT(mediaStatusChanged(QMediaPlayer::MediaStatus)));
    connect(mediaObject_ctrl_playback, SIGNAL(error(QMediaPlayer::Error)), this, SLOT(showError(QMediaPlayer::Error)));
    ...
    //创建系统盘图标
    trayIcon = new QSystemTrayIcon(QIcon(":/image/images/icon.png"), this);
...
void MyWidget::openFile()
{
    ...
    if (list.empty())
        return;
    //将打开的音乐文件添加到媒体源列表后
    QMediaContent cur_media;
    int old_size = sources.size();
    foreach(QString songstring, list)
    {
       cur_media = QMediaContent(QUrl::fromLocalFile(songstring));
    ...
    {
        mediaObject_ctrl_resolve->setMedia(sources[old_size]);
    }
}
...
void MyWidget::metaDataAvailableChanged(bool available)
{
    ...
    if (available)
    {
        ...
        //获取艺术家信息
        QTableWidgetItem * artistItem = new QTableWidgetItem(mediaObject_ctrl_resolve->metaData("Author").toString());
        artistItem->setFlags(artistItem->flags() & (~Qt::ItemIsEditable));

        //获取总时间信息
        qint64 totalTime = mediaObject_ctrl_resolve->metaData("Duration").toInt();
...
void MyWidget::setPaused()
{
    ...
    else
    {
        if (mediaObject_ctrl_playback->mediaStatus() == QMediaPlayer::NoMedia && !sources.empty())
            mediaObject_ctrl_playback->setMedia(sources[0]);
...
void MyWidget::setStop()
{
    ...
    if (sources.size() > 0)
        ...
        mediaObject_ctrl_playback->setMedia(QMediaContent());
    changeActionState();
}
...
void MyWidget::skipBackward()
{
    ...
    mediaObject_ctrl_playback->setMedia(sources[index - 1]);
    mediaObject_ctrl_playback->play();
}
...
void MyWidget::tableDoubleClicked(int row)
{
    ...
    mediaObject_ctrl_playback->setMedia(sources[row]);
    mediaObject_ctrl_playback->play();
}
...
{
    sources.clear();
    mediaObject_ctrl_resolve->setMedia(QMediaContent());
    changeActionState();
}
...
void MyWidget::stateChanged(QMediaPlayer::State state)
{
    ...
    case QMediaPlayer::PlayingState:
        ...
        if (lrc->mask_flag())
        {
            ...
            resolveLrc(mediaObject_ctrl_playback->currentMedia().request().url().toLocalFile());
        break;
    case QMediaPlayer::PausedState:
        ...
        topLabel->setText(mediaObject_ctrl_playback->currentMedia().request().url().fileName() + " 已暂停！");
        break;
    default: break;
    }
}

void MyWidget::mediaStatusChanged(QMediaPlayer::MediaStatus status)
{
    switch (status)
    {
        case QMediaPlayer::UnknownMediaStatus:
            topLabel->setText("媒体未知状况！");
            break;
...
void MyWidget::showError(QMediaPlayer::Error error)
{
    switch (error)
    {
        ...
        case QMediaPlayer::ResourceError:
            QMessageBox::warning(this, "错误", mediaObject_ctrl_playback->errorString());
            break;
...
void MyWidget::resolveLrc(const QString & sourceFileName)
{
    ...
    {
        lrc->setText(mediaObject_ctrl_playback->currentMedia().request().url().fileName() + " --- 未找到歌词文件");
        return;
    }
    //设置字符串编码
    ...
    foreach (QString oneLine, lines)
    {
        ...
        while (pos != -1)
        {
            QString cur_cap = rx.cap(0);
            QRegExp cur_rx;
            cur_rx.setPattern("\\d{2}(?=\\:)");
            cur_rx.indexIn(cur_cap, 0);
            int minute = cur_rx.cap(0).toInt();
            cur_rx.setPattern("\\d{2}(?=\\.)");
            cur_rx.indexIn(cur_cap, 0);
            int seconds = cur_rx.cap(0).toInt();
            cur_rx.setPattern("\\d{2}(?=\\])");
            cur_rx.indexIn(cur_cap, 0);
            int miliseconds = cur_rx.cap(0).toInt(); //歌词文件里面代表的是十毫秒
    ...
    if (lrcMap.empty())
    {
        lrc->setText(mediaObject_ctrl_playback->currentMedia().request().url().fileName() + " --- 歌词文件内容错误! ");
        return;
    }
}
